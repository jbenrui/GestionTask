...
function mapPackages(dependencies, devDependencies, specifiers, packages, inlineSpecifiers) {
    ...
    Object.entries(packages).forEach(([key, value]) => {
        const { resolution, engines } = value, dependencyDetails = tslib_1.__rest(value, ["resolution", "engines"]);
        // construct packageMeta object
        const meta = mapMetaInformation({ dependencies, devDependencies, specifiers }, inlineSpecifiers, key, dependencyDetails);
        // create new key
    ...
    for (let i = 0; i < packageNames.length; i++) {
        ...
        versions.forEach(({ packageMeta, resolution, engines }) => {
            packageMeta.forEach((meta) => {
                ...
                packages[key] = Object.assign({ resolution,
                    engines }, meta.dependencyDetails);
            });
        });
    }
